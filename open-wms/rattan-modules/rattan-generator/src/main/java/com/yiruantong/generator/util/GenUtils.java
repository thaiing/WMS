package com.yiruantong.generator.util;
...
import com.yiruantong.generator.config.GenConfig;
...
public class GenUtils {
  ...
  public static void initColumnField(GenTableColumn column, GenTable table) {
    ...
    if (arraysContains(GenConstants.COLUMNTYPE_STR, dataType) || arraysContains(GenConstants.COLUMNTYPE_TEXT, dataType)) {
    ...
    } else if (StringUtils.equals(GenConstants.TYPE_JSON, dataType)) {
      // JOSN类型
      column.setJavaType(GenConstants.TYPE_JSON_JAVA);
      ...
      String[] str = StringUtils.split(StringUtils.substringBetween(column.getColumnType(), "(", ")"), StringUtils.SEPARATOR_COMMA);
      if (str != null && str.length == 2 && Integer.parseInt(str[1]) > 0) {
        column.setJavaType(GenConstants.TYPE_BIGDECIMAL);
      }
      // 如果是tinyint
      else if (StringUtils.contains(column.getColumnType(), "tinyint")) {
    ...
    // 状态字段设置单选框
    if (StringUtils.endsWithIgnoreCase(columnName, "enable")) {
  ...
  public static String convertClassName(String tableName) {
    ...
    if (autoRemovePre && StringUtils.isNotEmpty(tablePrefix)) {
      String[] searchList = StringUtils.split(tablePrefix, StringUtils.SEPARATOR_COMMA);
