...
export function setTagsViewNameI18n(item: any) {
  ...
  if (query?.tagsViewName || params?.tagsViewName) {
    ...
    // 非自定义 tagsView 名称
    if (meta.isDynamic) {
      let path = item.fullPath || item.meta.dynamicPath;
      let r = router.getRoutes().find((f) => f.meta.dynamicPath === path);
      if (r) title = r?.meta.title;
    }
    tagsViewName = i18n.global.t(title);
  }
  return tagsViewName;
}
