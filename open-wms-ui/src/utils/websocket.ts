...
let HeartTimeOut = 10000; // 心跳超时时间 10000 = 10s
...
export const websocketonmessage = () => {
  websocket.onmessage = function (e: any) {
    ...
    if (e.data.indexOf('ping') > 0) {
      ...
      Session.set(Constants.WEB_SOCKET_SESSION_ID, json.webSocketSessionId);
      return;
    }
    console.log('websocket消息=', e.data);
    useNoticeStore().addNotice({
      message: isJSON(e.data) ? JSON.parse(e.data) : e.data,
      read: false,
