<template>
  <div v-if="isShowBreadcrumb" class="layout-navbars-breadcrumb flex-between">
...
import { reactive, computed, onMounted, ComponentInternalInstance } from 'vue';
...
onMounted(async () => {
  ...
  await initRouteSplit(route.path);
  window.setTimeout(() => {
    proxy.$forceUpdate();
  }, 1500);
});
