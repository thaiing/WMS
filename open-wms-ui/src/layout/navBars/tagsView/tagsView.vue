  ...
  <div class="layout-navbars-tagsview" :class="{ 'layout-navbars-tagsview-shadow': getThemeConfig.layout === 'classic' }">
    <el-scrollbar ref="scrollbarRef" @wheel.prevent="onHandleScroll">
      <ul class="layout-navbars-tagsview-ul" :class="setTagsStyle" ref="tagsUlRef">
        <li v-for="(v, k) in state.tagsViewList" :key="k" class="layout-navbars-tagsview-ul-li" :data-url="v.url" :class="{ 'is-active': isActive(v) }" @contextmenu.prevent="onContextmenu(v, $event)" @mousedown="onMousedownMenu(v, $event)" @click="onTagsClick(v, k)" :ref="(el) => setTagsRefs(el, k)">
          ...
          <template v-if="isActive(v)">
            ...
            <SvgIcon name="ele-Close" class="layout-navbars-tagsview-ul-li-icon layout-icon-active" v-if="!v.meta.isAffix" @click.stop="closeCurrentTagsView(getThemeConfig.isShareTagsView ? v.path : v.url)" />
          </template>
          <SvgIcon name="ele-Close" class="layout-navbars-tagsview-ul-li-icon layout-icon-three" v-if="!v.meta.isAffix" @click.stop="closeCurrentTagsView(getThemeConfig.isShareTagsView ? v.path : v.url)" />
        </li>
      </ul>
    </el-scrollbar>
...
// 定义变量内容
const tagsRefs: any = ref<RefType>([]);
...
const initTagsView = async () => {
  if (Session.get('tagsViewList') && getThemeConfig.value.isCacheTagsView) {
    ...
    await state.tagsViewRoutesList.map(async (v: RouteItem) => {
      if (v.meta?.isAffix && !v.meta.isHide) {
        v.url = setTagsViewHighlight(v);
        await addTagsViewList(v);
...
const solveAddTagsView = async (path: string, to?: RouteToFrom) => {
  // let current = state.tagsViewList.filter((v: RouteItem) => (v.meta?.dynamicPath || v.path) === path && isObjectValueEqual(to?.meta?.isDynamic ? (v.params ? v.params : null) : v.query ? v.query : null, to?.meta?.isDynamic ? (to?.params ? to?.params : null) : to?.query ? to?.query : null));
  let current = state.tagsViewList.filter((v: RouteItem) => v.meta?.menuId === to?.meta?.menuId).filter((v: RouteItem) => !v.meta?.dynamicPath || (v.meta?.dynamicPath && JSON.stringify(v.params) === JSON.stringify(to?.params)));
  if (current.length <= 0) {
    ...
    // let findItem = state.tagsViewRoutesList.find((v: RouteItem) => (v.meta?.dynamicPath || v.path) === path);
    let findItem = state.tagsViewRoutesList.find((v: RouteItem) => v.meta?.menuId === to?.meta?.menuId);
    if (!findItem) return false;
    if (findItem.meta.isAffix) return false;
    if (findItem.meta.isLink && !findItem.meta.isIframe) return false;
    to?.meta?.isDynamic ? (findItem.params = to.params) : (findItem.query = to?.query);
    findItem.url = setTagsViewHighlight(findItem);
    await addTagsViewList(findItem);
...
const singleAddTagsView = (path: string, to?: RouteToFrom) => {
  state.tagsViewList.forEach((v) => {
    if ((v.meta?.dynamicPath || v.path) === path && !isObjectValueEqual(to?.meta?.isDynamic ? (v.params ? v.params : null) : v.query ? v.query : null, to?.meta?.isDynamic ? (to?.params ? to?.params : null) : to?.query ? to?.query : null)) {
...
const addTagsView = (path: string, to?: RouteToFrom) => {
  let exitRoute = state.tagsViewRoutesList.find((item) => item.meta.dynamicPath === to?.path);
  if (to?.meta && exitRoute) {
    to.meta.menuId = exitRoute.meta.menuId;
    to.meta.dynamicPath = exitRoute.meta.dynamicPath;
    to.meta.isDynamicPath = exitRoute.meta.isDynamicPath;
    to.meta.title = exitRoute.meta.title;
    to.meta.activeMenu = exitRoute.meta.activeMenu;
  }
  // 防止拿取不到路由信息
  nextTick(async () => {
    ...
    if (to?.meta?.isDynamic) {
      ...
      if (state.tagsViewList.some((v: RouteItem) => v?.meta?.dynamicPath === to?.path)) {
        // 防止首次进入界面时(登录进入) tagsViewList 不存浏览器中
        addBrowserSetSession(state.tagsViewList);
        return false;
      }
      item = state.tagsViewRoutesList.find((v: RouteItem) => v?.meta?.dynamicPath === to?.path);
    } else {
...
const closeOtherTagsView = (path: string) => {
  if (Session.get('tagsViewList')) {
    state.tagsViewList = [];
    Session.get('tagsViewList').map(async (v: RouteItem) => {
      if (v.meta?.isAffix && !v.meta.isHide) {
        v.url = setTagsViewHighlight(v);
...
onBeforeMount(() => {
  ...
  mittBus.on('openShareTagsView', () => {
    if (getThemeConfig.value.isShareTagsView) {
      router.push(themeConfig.value.defaultHome);
      state.tagsViewList = [];
      state.tagsViewRoutesList.map(async (v: RouteItem) => {
        if (v.meta?.isAffix && !v.meta.isHide) {
          v.url = setTagsViewHighlight(v);
          await addTagsViewList(v);
        }
  ...
  // 监听关闭当前窗口
  mittBus.on('closeCurrentTag', () => {
    debugger;
    let path = state.routeActive;
...
const setTagsRefs = (el: any, k: any) => {
  if (el) {
    tagsRefs[k] = el;
  }
};

const addTagsViewList = async (item: RouteItem<any>) => {
  let name: any = item.name;
  if (item?.params?.id) {
    name = 'amis-amis-engine-' + item.params.id;
  }
  if (item?.params?.bizId) {
    name += '-' + item.params.bizId;
  }
  // item.name = name;
  await state.tagsViewList.push({ ...item });
};

const pushRouter = (item: RouteItem<any>) => {
  let path = item.path;
  if (item.meta?.isDynamic) {
    if (item?.params?.id) {
      path = item.meta.dynamicPath as string;
    }
    if (item?.params?.bizId) {
      path = `/amis/amis-engine/${item?.params?.id}/${item?.params?.bizId}/订单编辑`;
    }
  }
  let data: any = {
    path: path,
