...
<script setup lang="ts" name="system-task-task">
...
const push = async () => {
  if (!state.dataListSelections.length) {
    proxy.$message.error('请选择要推送的数据');
    return;
  }

  let ids = state.dataListSelections.map((m) => m.wcsTaskId);
  const url = '/system/task/task/push/' + ids.join(',');
  const params = {};
