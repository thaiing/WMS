          ...
          <el-form class="form-tool" @submit.stop.prevent>
            ...
            <el-form-item class="padding-right-0 margin-bottom-0">
              <template v-if="state.filterText">
                <div v-for="item in state.filterDataList" class="filter-box" :class="item.importId === state.currentNode.importId ? 'active' : ''" @click="nodeClick(item)">
                  <div class="filter-title">{{ item.importName }}</div>
              ...
              <el-scrollbar v-else :noresize="false" :native="false" wrap-class="tree scrollbar-wrap">
                <!--数据tree-->
                <el-tree ref="tree" :data="state.dataTree" :expand-on-click-node="false" :load="(node:any, resolve:any) => loadTreeNode(node, resolve)" :filter-node-method="filterTreeNode" :props="state.props" :default-expand-all="false" highlight-current lazy node-key="importId" @node-click="nodeClick">
...
<script setup lang="ts" name="system-dataHandler-import">
...
const loadTreeNode = async (node: any, resolve: any) => {
  ...
  if (res.result) {
    ...
    res.data.forEach((element: any) => {
      element.label = element['importName'];
...
const nodeClick = (data: any, node?: any, el?: any) => {
  if (editorRef) {
    ...
    editorRef.value.loadEditData(data.importId);
  } else {
...
const treeRefresh = async () => {
  ...
  let url = '/system/dataHandler/import/searchTree/' + encodeURI(state.filterText);
  let params = {};
...
const onSaveAfter = (formData: any) => {
  ...
  if (node.importId !== formData.importId) {
    let parentNode = proxy.$refs.tree.getNode(node.parentId);
    if (state.addLevel === 'son') {
      parentNode = proxy.$refs.tree.getNode(node.importId);
    } else {
      node.importName = formData.importName;
      node.label = formData.importName;
    }
    proxy.$refs.tree.append(
      {
        ...
        label: formData.importName,
      },
      parentNode
    );
    proxy.$refs.tree.setCurrentKey(formData.importId);
    nodeClick(formData);
  } else {
    node.importName = formData.importName;
    node.label = formData.importName;
...
const createBrotherNode = (formData: any, details: any) => {
  formData.importId = 0;
  formData.importName = null;
  formData.templatePath = null;
  formData.execSql = null;
  state.addLevel = 'brother';
};

// 创建子级同级
const createSonNode = (formData: any, details: any) => {
  formData.parentId = formData.importId;
  formData.importId = 0;
  formData.importName = null;
  state.addLevel = 'son';
};
...
const deleteNode = async (formData: any, details: any) => {
  const _deleteNode = async () => {
    let url = '/system/dataHandler/import/deleteTreeNode';
    let params = {
      tableName: 'sys_import',
      keyName: 'importId',
      keyValue: formData.importId,
      nodeName: 'importName',
      parentName: 'parentId',
      extendColumns: '',
    };
    ...
    if (res.result) {
      ...
      proxy.$refs.tree.remove(formData.importId);
      if (nextNode) {
        proxy.$refs.tree.setCurrentKey(nextNode.data.importId);
    ...
    proxy.$refs.tree.append(
      {
        hasChild: false,
        isLeaf: true,
        importId: dataInfo.importId,
        sql: formData.sql,
        importName: dataInfo.importName,
        label: dataInfo.importName,
      },
      parentNode
    );
    proxy.$refs.tree.setCurrentKey(dataInfo.importId);
    nodeClick(dataInfo);
  });
};
