          ...
          <el-form class="form-tool" @submit.stop.prevent>
            ...
            <el-form-item class="padding-right-0 margin-bottom-0">
              <template v-if="state.filterText">
                <div v-for="item in state.filterDataList" class="filter-box" :class="item.dropdownId === state.currentNode.dropdownId ? 'active' : ''" @click="nodeClick(item)">
                  <div class="filter-title">{{ item.cnName }}</div>
              ...
              <el-scrollbar v-else :noresize="false" :native="false" wrap-class="tree scrollbar-wrap">
                <!--数据tree-->
                <el-tree ref="tree" :data="state.dataTree" :expand-on-click-node="false" :load="(node:any, resolve:any) => loadTreeNode(node, resolve)" :filter-node-method="filterTreeNode" :props="state.props" :default-expand-all="false" highlight-current lazy node-key="dropdownId" @node-click="nodeClick">
...
<script setup lang="ts" name="system-dataHandler-dropdown">
...
const nodeClick = (data: any, node?: any, el?: any) => {
  if (editorRef) {
    ...
    editorRef.value.loadEditData(data.dropdownId);
  } else {
...
const treeRefresh = async () => {
  ...
  let url = '/system/dataHandler/dropdown/searchTree/' + encodeURI(state.filterText);
  let params = {};
...
const onSaveAfter = (formData: any) => {
  ...
  if (node.dropdownId !== formData.dropdownId) {
    let parentNode = proxy.$refs.tree.getNode(node.parentId);
    if (state.addLevel === 'son') {
      parentNode = proxy.$refs.tree.getNode(node.dropdownId);
    } else {
    ...
    proxy.$refs.tree.setCurrentKey(formData.dropdownId);
    nodeClick(formData);
  } else {
    node.cnName = formData.cnName;
    node.label = formData.cnName;
...
const createSonNode = (formData: any, details: any) => {
  formData.parentId = formData.dropdownId;
  formData.dropdownId = 0;
  formData.cnName = null;
  state.addLevel = 'son';
};
...
const deleteNode = async (formData: any, details: any) => {
  const _deleteNode = async () => {
    let url = '/system/dataHandler/dropdown/deleteTreeNode';
    let params = {
      tableName: 'sysDropdown',
      keyName: 'dropdownId',
      keyValue: formData.dropdownId,
      nodeName: 'cnName',
      parentName: 'parentId',
      extendColumns: '',
    };
    ...
    if (res.result) {
      ...
      proxy.$refs.tree.remove(formData.dropdownId);
      if (nextNode) {
        proxy.$refs.tree.setCurrentKey(nextNode.data.dropdownId);
...
const copyNode = (formData: any, details: any) => {
  ...
  editorRef.value.copy((res: any) => {
    ...
    proxy.$refs.tree.append(
      {
        hasChild: false,
        isLeaf: true,
        dropdownId: dataInfo.dropdownId,
        sql: formData.sql,
    ...
    proxy.$refs.tree.setCurrentKey(dataInfo.dropdownId);
    nodeClick(dataInfo);
  });
};
