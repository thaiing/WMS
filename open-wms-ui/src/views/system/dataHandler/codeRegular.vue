    ...
    <split-pane :max-width="500" :default-width="260" split="vertical">
          ...
          <el-form class="form-tool" @submit.stop.prevent>
            ...
            <el-form-item class="padding-right-0 margin-bottom-0">
              <template v-if="state.filterText">
                <div v-for="item in state.filterDataList" class="filter-box" :class="item.regularId === state.currentNode.regularId ? 'active' : ''" @click="nodeClick(item)">
                  <div class="filter-title">{{ item.menuName }}</div>
              ...
              <el-scrollbar v-else :noresize="false" :native="false" wrap-class="tree scrollbar-wrap">
                <!--数据tree-->
                <el-tree ref="tree" :data="state.dataTree" :expand-on-click-node="false" :load="(node:any, resolve:any) => loadTreeNode(node, resolve)" :filter-node-method="filterTreeNode" :props="state.props" :default-expand-all="false" highlight-current lazy node-key="regularId" @node-click="nodeClick">
      ...
      <template #paneR>
        <div class="right-container">
          <!--数据编辑器Editor-->
          <yrt-editor :ref="editorInfo.refName" :editor-type="state.dataOptions.editorType" v-model:action="editorInfo.action" :data-list-ref="dataListRefName" v-model:config="state.editorOptions.config" :data-options="state.dataOptions" :editorOptions="state.editorOptions" :detail-button-click="detailButtonClick" :auth-nodes="state.authNodes" @onSaveAfter="onSaveAfter">
            <!--自定义按钮插槽-->
...
<script setup lang="ts" name="system-dataHandler-codeRegular">
...
const nodeClick = (data: any, node?: any, el?: any) => {
  if (editorRef) {
    ...
    editorRef.value.loadEditData(data.regularId);
  } else {
...
const treeRefresh = async () => {
  ...
  let url = '/system/dataHandler/codeRegular/searchTree/' + encodeURI(state.filterText);
  let params = {};
...
const onSaveAfter = (formData: any) => {
  ...
  if (node.regularId !== formData.regularId) {
    let parentNode = proxy.$refs.tree.getNode(node.parentId);
    if (state.addLevel === 'son') {
      parentNode = proxy.$refs.tree.getNode(node.regularId);
    } else {
    ...
    proxy.$refs.tree.setCurrentKey(formData.regularId);
    nodeClick(formData);
  } else {
    node.menuName = formData.menuName;
    node.label = formData.menuName;
...
const createSonNode = (formData: any, details: any) => {
  formData.parentId = formData.regularId;
  formData.regularId = 0;
  formData.menuName = null;
  state.addLevel = 'son';
};
...
const deleteNode = async (formData: any, details: any) => {
  const _deleteNode = async () => {
    let url = '/system/dataHandler/codeRegular/deleteTreeNode';
    let params = {
      tableName: 'sysCodeRegular',
      keyName: 'regularId',
      keyValue: formData.regularId,
      nodeName: 'menuName',
      parentName: 'parentId',
      extendColumns: '',
    };
    ...
    if (res.result) {
      ...
      proxy.$refs.tree.remove(formData.regularId);
      if (nextNode) {
        proxy.$refs.tree.setCurrentKey(nextNode.data.regularId);
...
const copyNode = (formData: any, details: any) => {
  ...
  editorRef.value.copy((res: any) => {
    ...
    proxy.$refs.tree.setCurrentKey(dataInfo.regularId);
    nodeClick(dataInfo);
  });
};
