<template>
	<el-dialog v-model="currentDialogVisible" :top="common.getDialogTop()" title="修改email" class="dialog-container" width="480px" draggable overflow append-to-body destroy-on-close :close-on-click-modal="false" @open="onOpen">
		<div class="fx-solution-center-wrapper">
			...
			<el-form v-else ref="ruleFormRef" :model="regForm" :rules="rules" label-width="auto" class="register-form" size="large" status-icon>
				<el-form-item label="当前Email：">
					{{ props.email }}
				</el-form-item>
				<el-form-item label="新Email：" prop="email">
					<el-input v-model="regForm.email" placeholder="请输入新Email" style="width: 228px" />
				</el-form-item>
...
const clickGetCode = async (formEl: FormInstance | undefined) => {
	...
	await formEl.validateField('email', (valid, fields) => {
		if (valid) {
			getCode();
		} else {
...
const getCode = async () => {
	...
	let url = '/resource/email/code/' + props.email;
	let params = {};
...
const modifyEmail = async () => {
	state.smsBtnDisabled = true;
	let url = '/system/permission/user/modifyEmail';
	let params = regForm;
	const [err, res] = await to(postData(url, params));
	state.smsBtnDisabled = false;
	timeLong = 0;
