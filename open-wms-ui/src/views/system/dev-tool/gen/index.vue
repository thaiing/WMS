	...
	<div class="ma-20">
		...
		<el-card shadow="never">
			<template #header>
				<el-row :gutter="10" class="mb8">
					...
					<el-col :span="1.5">
						<el-button type="success" plain icon="Edit" :disabled="single" @click="handleEditTable()">修改</el-button>
					</el-col>
					<el-col :span="1.5">
		...
		<el-dialog :title="dialog.title" v-model="dialog.visible" width="80%" top="5vh" append-to-body class="scrollbar">
			<el-tabs v-model="preview.activeName">
				<el-tab-pane v-for="(value, key) in preview.data" :label="(key as any).substring((key as any).lastIndexOf('/') + 1, (key as any).indexOf('.vm'))" :name="(key as any).substring((key as any).lastIndexOf('/') + 1, (key as any).indexOf('.vm'))" :key="value">
...
<script setup name="system-dev-tool-gen" lang="ts">
...
onActivated(() => {
	...
	if (time != null && time != uniqueId.value) {
		...
		queryParams.value.pageIndex = Number(route.query.pageIndex);
		dateRange.value = ['', ''];
...
const handleEditTable = (row?: TableVO) => {
	...
	router.push({
		name: 'system-dev-tool-gen-edit',
		params: {
			tableId: tableId,
			tagsViewName: '表结构信息编辑',
		},
	});

	// router.push({ path: '/system/dev-tool/gen/edit/' + tableId, query: { pageIndex: queryParams.value.pageIndex } });
};
/** 删除按钮操作 */
