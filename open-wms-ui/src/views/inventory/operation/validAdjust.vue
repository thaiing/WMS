...
<script setup lang="ts" name="inventory-operation-validAdjust">
...
const setTotal = (field: any, detailRows: Array<any>) => {
  ...
  detailRows.forEach((item) => {
    ...
    // 目标到期日期
    if (item.produceDateTarget) {
      item.limitDateTarget = moment(item.produceDate_Target).add(item.shelfLifeDay, 'days').format('YYYY-MM-DD');
    } else {
      item.limitDateTarget = null;
    }
...
const multiAuditing = async (selections: Array<any>) => {
  const url = '/inventory/operation/validAdjust/multiAuditing';
  ...
  let ids = selections.map((item) => item.validAdjustId);
