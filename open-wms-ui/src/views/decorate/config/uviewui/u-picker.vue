      ...
      <el-tab-pane label="参数设置" name="t1" class="tab-pane">
        <el-form ref="formRef" :model="config" label-width="100px">
          ...
          <el-form-item label="主键字段">
            <el-input v-model="config.keyProp"></el-input>
          </el-form-item>
          <el-form-item label="输入框类型">
            <el-select v-model="config.type" placeholder="请选择" @change="(val) => dataTypeChange(val)">
          ...
          <el-form-item label="选项">
            <el-radio-group v-model="config.options.remote" size="small" style="margin-bottom: 10px">
              <el-radio-button :label="'bindDropdown'">下拉框ID</el-radio-button>
              <el-radio-button :label="false">静态数据</el-radio-button>
              <el-radio-button :label="true">远端数据</el-radio-button>
            </el-radio-group>
            <template v-if="config.options.remote === true">
              <div>
                <el-input v-model="config.options.remoteFunc" size="small" style="">
                  <template #prepend>远端方法</template>
                </el-input>
                <div></div>
                <el-input v-model="config.options.props.value" size="small" style="">
                  <template #prepend>值</template>
                </el-input>
                <div></div>
                <el-input v-model="config.options.props.label" size="small" style="">
                  <template #prepend>标签</template>
                </el-input>
              </div>
            </template>
            <template v-else-if="config.options.remote === 'bindDropdown'">
              <el-input v-model.number="config.options.dropdownId" type="number" size="small">
                <template #prepend>关联下拉框ID</template>
              </el-input>
            </template>
            <template v-else-if="config.options.remote === false">
              <el-radio-group v-model="config.options.defaultValue">
                <draggable :list="config.options.options" item-key="index" group="options" ghostClass="ghost" handle=".drag-item" tag="ul">
                  <template #item="{ element, index }">
                    <li :key="index">
                      ...
                      <el-button circle plain type="danger" size="small" style="padding: 4px; margin-left: 5px" @click="handleOptionsRemove(index)"><i class="el-icon-yrt-icon_cut"></i></el-button>
                    </li>
                  </template>
                </draggable>
              </el-radio-group>
...
export default {
  ...
  setup() {
    ...
    let method = {
      // 编辑框类型改变
      dataTypeChange,
      handleOptionsRemove(index: any) {
        proxy.config.options.options.splice(index, 1);
      },
      handleAddOption() {
        proxy.config.options.options.push({
          value: '新选项',
          label: '新选项',
        });
      },
