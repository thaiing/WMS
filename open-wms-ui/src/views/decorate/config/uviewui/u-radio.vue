      ...
      <el-tab-pane label="参数设置" name="t1" class="tab-pane">
        <el-form ref="formRef" :model="config" label-width="100px">
          ...
          <el-form-item label="数据类型">
            <el-select v-model="config.dataType" style="width: 100%" placeholder="请选择">
              <el-option v-for="(item, index) in dataTypeList" :key="index" :label="item" :value="item"> </el-option>
            </el-select>
          </el-form-item>
          <el-form-item label="布局方式">
            <el-radio-group v-model="config.inline">
              <el-radio-button :label="false">行内</el-radio-button>
              <el-radio-button :label="true">块级</el-radio-button>
            </el-radio-group>
          </el-form-item>
          <el-form-item label="数据">
            <el-radio-group v-model="config.defaultValue">
  ...
  setup() {
    ...
    let method = {
      ...
      handleOptionsRemove(index: any) {
        proxy.config.radioOptions.splice(index, 1);
      },
      handleAddOption() {
        let content = '新选项' + (proxy.config.radioOptions.length + 1);
        proxy.config.radioOptions.push({
          value: content,
          label: content,
        });
      },
