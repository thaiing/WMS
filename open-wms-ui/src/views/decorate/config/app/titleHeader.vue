    ...
    <el-tabs v-model="tabActive" class="tabs-container">
      <el-tab-pane label="内容设置" name="t1" class="tab-pane">
        ...
        <el-badge value="x" type="primary" class="badge" @click.stop.prevent="clearImage()">
          <el-image class="img" :src="config.content.image" :initial-index="1" fit="contain" @click.stop.prevent="selectFile()">
            <template #error>
        ...
        <el-form ref="formRef" :model="config" label-width="100px">
          ...
          <el-form-item label="连接">
            <el-input v-model="config.content.url">
              <template #suffix>
          ...
          <el-form-item label="显示更多">
            <el-switch v-model="config.content.showMore" />
          </el-form-item>
        </el-form>
      </el-tab-pane>
...
export default {
  ...
  setup() {
    ...
    let method = {
      ...
      // 清除图片
      clearImage() {
        proxy.config.content.image = null;
      },
      // 选择文件
      selectFile() {
    ...
    onMounted(async () => {
      ...
      proxy.mittBus.on('onSelectFile', (item: any) => {
        if (item) {
          proxy.config.content.image = item.url;
