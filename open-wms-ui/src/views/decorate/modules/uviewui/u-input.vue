<template>
	<div class="module-container" :style="{ 'background-color': config.style?.bgColor, 'margin-top': config.style.maginTop ? config.style.maginTop + 'px' : 0, padding: config.style?.bgMargin ? config.style.bgMargin + 'px' : 0 }">
		<el-form ref="form" :model="config" label-width="90px" class="widget-view">
			<el-form-item :label="config.label" :label-width="config.labelWidth ? config.labelWidth : '90px'">
				<el-input v-model="config.defaultValue" :style="{ width: config.width ? config.width : '100%' }" :placeholder="config.placeholder" :disabled="!!config.readonly"></el-input>
			</el-form-item>
		</el-form>
	</div>
</template>
