	...
	<div class="module-container" :style="{ 'background-color': config.style?.bgColor, 'margin-top': config.style.maginTop ? config.style.maginTop + 'px' : 0, padding: config.style?.bgMargin ? config.style.bgMargin + 'px' : 0 }">
		<el-form ref="form" :model="form" label-width="90px" class="widget-view">
			<el-form-item :label="config.label" :label-width="config.options.labelWidth ? config.options.labelWidth : '90px'">
				<el-select v-model="form.value" :placeholder="config.options.placeholder || '请选择'" :style="{ width: config.options.width ? config.options.width : '100%' }" :disabled="!!config.options.readonly">
