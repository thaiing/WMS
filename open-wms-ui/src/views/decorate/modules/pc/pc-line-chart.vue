...
const initChart = () => {
	...
	let series = props.config.series.map((item: any) => {
		return {
			...
			lineStyle: { color: item.lineStyleColor },
			itemStyle: { color: item.itemStyleColor, borderColor: item.itemStyleBorderColor },
			areaStyle: {
				color: new echarts.graphic.LinearGradient(0, 0, 0, 1, [
					{ offset: 0, color: item.areaStyleColor1 },
					{ offset: 1, color: item.areaStyleColor2 },
				]),
			},
			emphasis: {
				focus: item.emphasisFocus || 'none',
			},
		};
	});
