...
<script setup lang="ts" name="basic-consignor-consignorStoreAddress">
...
base.onChange = (ref: any, val: any, field: any, formData: any) => {
  ...
  if (field.options.prop === 'provinceName') {
    masterData.value.cityName = null;
    masterData.value.cityId = null;
    masterData.value.regionName = null;
    masterData.value.regionId = null;
    // 改变市级下拉框
...
base.onSaveBefore = (formData: any) => {
  var pickupStartTime = moment(formData.pickupStartTime);
  var pickupEndTime = moment(formData.pickupEndTime);

  if (pickupStartTime && pickupEndTime && pickupStartTime > pickupEndTime) {
    proxy.$message.error('提货开始时间不能大于结束时间！');
    return false;
  }

  var deliveryStartTime = moment(formData.deliveryStartTime);
  var deliveryEndTime = moment(formData.deliveryEndTime);

  if (deliveryStartTime && deliveryEndTime && deliveryStartTime > deliveryEndTime) {
    proxy.$message.error('配送开始时间不能大于结束时间！');
    return false;
  }
  return true;
};
