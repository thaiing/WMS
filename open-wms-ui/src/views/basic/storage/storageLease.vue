...
<script setup lang="ts" name="basic-storage-storageLease">
...
base.onSaveBefore = (formData: any) => {
	var start = moment(formData.startDate);
	var end = moment(formData.endDate);
	if (start && end && start > end) {
		proxy.$message.error('租赁开始日期不能大于结束日期！');
		return false;
	}
	return true;
};
